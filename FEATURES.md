...
| Gradient backgrounds | Yes | Yes | Yes | Not documented | html2canvas and html2canvas-pro document `linear-gradient()` and `radial-gradient()` support. `layout2vector` supports gradients across multiple writers. |
...
| Modern CSS color functions (`color()`, `lab()`, `lch()`, `oklab()`, `oklch()`) | Partial | No | Yes | Not documented | `layout2vector` has tested parsing for `color(srgb ...)`, `lab()`, `lch()`, `oklab()`, and `oklch()`. Broader `color()` profiles such as `display-p3` are still not covered. |
| `object-fit` on `<img>` | Partial | No | Yes | Not documented | `layout2vector` has direct test coverage for `contain`, `cover`, and `scale-down` on extracted `<img>` content. Custom `object-position` handling is still not explicitly covered. |
| CSS `clip-path` shapes | Partial | Not documented | Yes | Not documented | `layout2vector` has tested support for `inset()`, `circle()`, `ellipse()`, and `polygon()` across the Image/Canvas, HTML, SVG, PDF, and EMF writers. `path()` and more complex nested clip composition are still missing. |
